...
- 이 문서를 `plan/active/streaming_pipeline.md`로 등록한다.
- `todo.md` Active에 master plan을 연결한다.
...
- Python model/stream 모듈과 연결하는 방식은 Phase 5 세부 계획에서 결정한다.

연결 방식 후보:

```text
1. C++ replay -> event JSONL/NDJSON 출력 -> Python consumer 처리
2. C++ replay -> micro-batch 단위 Python CLI 호출
3. C++ replay -> local pipe/socket으로 Python worker에 event 전달
```
...
- [ ] `plan/active/streaming_pipeline.md`가 master plan 역할을 한다.
- [ ] `todo.md` Active에 이 계획이 연결되어 있다.
- [ ] 각 phase가 별도 세부 계획과 승인 후 진행되는 방식이 문서화되어 있다.
- [ ] full streaming training을 하지 않는다는 범위가 명확하다.
- [ ] canonical event embedding 계약과 기존 overlap-window 방식의 문제가 문서화되어 있다.
- [ ] C++ replay engine과 dashboard 연동이 master plan 범위에 포함되어 있다.
